...
class DreamBoothDataset(Dataset):
    def __init__(
        ...
        data_df_path,
        fds_args={
            "annotations_file":"familydataset/family_dataset.csv",
            "img_dir":"familydataset"
        },
        size=1024,
        ...
        if not self.data_df_path.exists():
            raise ValueError("`data_df_path` doesn't exist.")

        # Load images.
        dataset = FamilyDataset(**fds_args)
        #dataset = load_dataset(dataset_name, split="train")
        instance_images = [sample[0] for sample in dataset]
